...
def run_adaptive_rmu(
    ...
    path = f"checkpoints/rm/adaptive_rmu/{args.model_name_or_path}_alpha-{int(args.alpha[0])}-{int(args.alpha[0])}_batches-{num_batches}_layer-{args.layer_id}_scale-{float(args.scale[0])}-{float(args.scale[1])}_nu-{args.nu}"
...
def get_args():
    ...
    parser.add_argument("--steering_coeffs", type=str, default="1.0,1.0", help="Steer vector weight in order of topic")
    parser.add_argument('--scale', type=str, default="3.0,3.0")
    parser.add_argument('--nu', type=float, default=0.0, help="noise magnitude")
